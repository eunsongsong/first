...
print( PROJECT_DIR )
# c:\Users\student\Desktop\py_projects\first
...
env.key_filename = 'song.pem' # 동일 폴더 위치
...
def _setup():pass
    ...
    # 프로젝트별로: 필요한 패키지를 설치하여
    # 상호 프로젝트간 영향을 받이 않게 만드는 방식(가상환경) 
    ...
    # 구동 
    # C:\~\py_projects\env\Scripts\python run.py 


# 7-2. apt(우분투 상) 패키지를 업데이트 여부 확인 및 업데이트
def _init_apt():
    yn = input('ubuntu linux update ok?: [y/n]')en
...
def _making_virtualenv():
    ...
    if not exists('/.virtualenvs'):
        ...
        cmd = '''
            ...
            export VIRTUALENVWRQPPER_PYTHON="$(command \which python3)"
            # shell 실행
...
def _git_update():
    if exists(project_folder + '/.git'): # 깃트가 존재하면
        # first 폴더로 이동 그리고 저장소로부터 fetch를 해서 최신 정보를 가져온다
        run('cd %s && git fetch' % (project_folder,))
    else: # 깃트가 존재하지 않으면
          # 저장소로부터 최초로 프로젝트를 받아온다
        run('git clone %s %s' % (REPO_URL, project_folder))
    # 기트의 커밋된 로그를 최신으로 한개 가져온다 그것의 번호를 리턴
    # local: git log -n 1 --format=%H => 238473278223543
...
def _virtualenv_update():
    ...
    # /home/ubuntu/.virtualenvs/first/bin/pip
    ...
    # 상관 없이 수행 => 필요한 python 모듈을 설치한다 (이 가상환경에게만 적용)
    # /home/ubuntu/.virtualenvs/first/bin/pip install -r /home/ubuntu/first/requirement.txt
